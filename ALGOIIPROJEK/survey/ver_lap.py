...
def lapangan():
    ...
    print("╔" + "═" * 50 + "╗")
    print(f"║" + f"{'Daftar Calon yang Lolos untuk Survei:':^50}" + "║")
    print(f"╠" + "═" * 50 + "╣")
    daftar_tampil = hasil_fil.to_dict('records')
    for idx, row in enumerate(daftar_tampil):
        print(f"║" + f"{idx + 1}. ID Pengajuan: {row['id_pengajuan']}, Status: {row['status_seleksi']:<19}" + "║")
    print("╚" + "═" * 50 + "╝")
    opsi = input("Masukkan nomor urut yang akan disurvey (kosongkan dan tekan Enter untuk kembali): ")
    if not opsi:
        print("Kembali.")
        return
    ...
    print("╔" + "═" * 65 + "╗")
    print(f"║" + f"{'Data dari Bapak/Ibu ' f"{calon.get('nama')}":<65}" + "║")
    print(f"╠" + "═" * 65 + "╣")
    header_baru = {
    ...
    for key, value in data_p[0].items():
        ...
        print(f"║" + f"{final:<35}: {value:<28}" + "║")
    print("╚" + "═" * 65 + "╝")

    valid = input("Apakah data benar dan valid? (ya/tidak): ").lower()
    catatan = input("Catatan survei (jika ada): ")

    hasil.loc[hasil['id_pengajuan'] == calon['id_pengajuan'], 'valid'] = bool(valid == 'ya')
    hasil.loc[hasil['id_pengajuan'] == calon['id_pengajuan'], 'catatan'] = str(catatan)
